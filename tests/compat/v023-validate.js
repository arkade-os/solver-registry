...
//
// @arkade-os/solver-discovery 0.2.3, dist/validate.js, fetched from npm 2026-09-09
...
import { AMOUNT_PATTERN, ASSET_KEYS, CORRIDOR_KEYS, CORRIDORS, LIMIT_KEYS, MAX_ASSET_DECIMALS, MAX_RELAYS, isAmount, isCorridor, isNetwork, isRfqMarket, marketCorridor, pairSideLabel, } from "./v023-types.js";
...
function checkPattern(errors, path, v, re, message) {
    if (typeof v !== "string" || !re.test(v))
        add(errors, path, message);
}
function checkIntRange(errors, path, v, min, max) {
    if (!isInt(v) || v < min || v > max)
        add(errors, path, `must be an integer in ${min}..${max}`);
}
function checkStringLength(errors, path, v, min, max) {
    if (typeof v !== "string" || v.length < min || v.length > max) {
        add(errors, path, `must be a string of length ${min}..${max}`);
    }
}
function checkAllowedKeys(errors, path, obj, allowed) {
    for (const key of Object.keys(obj)) {
        if (!allowed.has(key))
            add(errors, `${path}/${key}`, "is not an allowed property");
    }
}
const ASSET_KEY_SET = new Set(ASSET_KEYS);
const PRICE_FEED_SCHEMA_KEYS = new Set(["type", "price_path"]);
function checkAsset(errors, path, v, strict) {
    if (!isObject(v)) {
        add(errors, path, "must be an object");
        return;
    }
    if (strict)
        checkAllowedKeys(errors, path, v, ASSET_KEY_SET);
...
function checkPriceFeedSchema(errors, path, v, strict) {
    if (!isObject(v)) {
        add(errors, path, "must be an object");
        return;
    }
    if (strict)
        checkAllowedKeys(errors, path, v, PRICE_FEED_SCHEMA_KEYS);
    if (v.type !== "json")
        add(errors, `${path}/type`, 'must be "json"');
...
const LIMIT_SIDES = [LIMIT_KEYS.base, LIMIT_KEYS.quote];
/**
 ...
 * are skipped here.
 */
export function marketLimitErrors(market) {
    const errors = [];
    let checkedSides = 0;
    let enabledSides = 0;
    for (const { min: minKey, max: maxKey } of LIMIT_SIDES) {
        ...
        if (!isAmount(minRaw) || !isAmount(maxRaw))
            continue;
        checkedSides++;
...
export function marketPairError(market) {
    const base = market.base_asset?.ticker;
    const quote = market.quote_asset?.ticker;
    if (typeof market.pair !== "string" || typeof base !== "string" || typeof quote !== "string") {
        return null;
    }
    const expected = `${pairSideLabel(marketCorridor(market, "base"), base)}/${pairSideLabel(marketCorridor(market, "quote"), quote)}`;
    return market.pair === expected ? null : `pair "${market.pair}" does not match the sides' labels "${expected}"`;
}
const FEED_KEYS = ["price_feed", "price_feed_schema", "price_decimals"];
/**
...
export function marketCorridorErrors(market) {
    ...
    const baseId = market.base_asset?.id;
    const quoteId = market.quote_asset?.id;
    if (typeof baseId !== "string" || typeof quoteId !== "string")
        return errors;
 ...
 */
function checkMarket(errors, path, v, strict) {
    if (!isObject(v)) {
        add(errors, path, "must be an object");
        return;
    }
    if (strict)
        checkAllowedKeys(errors, path, v, MARKET_KEYS);
    checkPattern(errors, `${path}/pair`, v.pair, PAIR, 'must be "<base>/<quote>" where a non-arkade side is corridor-prefixed, e.g. "BTC/lightning:BTC"');
    ...
    if (v.fee_flat !== undefined) {
        checkPattern(errors, `${path}/fee_flat`, v.fee_flat, AMOUNT_PATTERN, "must be a decimal string of quote-asset atomic units");
    }
    for (const message of marketCorridorErrors(v))
        add(errors, path, message);
    ...
    for (const message of marketLimitErrors(v))
        add(errors, path, message);
}
/** Whether any of a card's markets has a non-arkade corridor (shape-defensive). */
export function cardHasRfqMarket(card) {
...
export function cardRfqErrors(card) {
    if (!cardHasRfqMarket(card))
        return [];
    const errors = [];
    for (const key of ["discovery_pubkey", "transports"]) {
...
function checkTransports(errors, path, v) {
    if (v === undefined)
        return;
    if (!isObject(v)) {
        add(errors, path, "must be an object");
...
function checkIndexMarket(errors, path, v) {
...
export function validateCard(input) {
    ...
    for (const message of cardRfqErrors(input))
        add(errors, "/", message);
    return errors.length === 0 ? { ok: true, errors: [], value: input } : { ok: false, errors };
}
/**
 ...
 */
export function validateIndex(input, expectedNetwork) {
    if (!isObject(input)) {
        return { ok: false, errors: ["/ must be an object"] };
    }
    const errors = [];
    if (input.version !== 0)
        add(errors, "/version", "must be 0 (unknown index version)");
