...
Where a maker prices from the card rather than from a quote, the card is the whole pricing contract: it must carry every input the solver's own check applies, or the maker computes terms the solver then refuses. `solver_fee` is that contract, keyed by the side the maker **deposits**. That key is the point: it fixes the units for everything beneath it, so the flat part needs no denominator and nothing is converted through the price — and because each side carries its own `bps`, the spread may differ by direction, which one market-level `fee_bps` cannot express. Exactly one key applies per swap, so the two are alternatives and never a sum. `fee_bps` remains required and MUST equal the widest spread declared here: a reader predating `solver_fee` applies it to both directions, and understating one would have that reader compute a payout the solver refuses, after the offer is funded on chain. `solver_fee` therefore supersedes `fee_flat` rather than adding to it, so a card may carry both while migrating. That migration is safe for the SPREAD only, and for the reason just given: a spread scales with the trade, so one widest `fee_bps` dominates both directions. The flat part has no such projection, because `fee_flat` is quote-denominated while `solver_fee.base.flat` is not, and converting between them needs the price — which the card carries only as a feed. A market declaring `solver_fee.base.flat` therefore CANNOT be priced by a reader predating that field: `fee_flat` projects the quote side alone, so such a reader understates the charge and computes exactly the payout the solver refuses once the offer is funded. Readers MUST implement `solver_fee` before pricing any market that declares one. `charges_delivered_carrier` says whether the solver funds the dust carrier an Arkade asset rides on out of the maker's deposit. Both `solver_fee.flat` and the carrier come off the DEPOSIT before the spread, matching the solver. The carrier's amount is not on the card: it is the Arkade Service's dust, both parties to an Arkade-settled swap share that Service, and a live figure beats a cached one.
