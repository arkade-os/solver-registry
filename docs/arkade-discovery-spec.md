...
Anyone can run a registry: it's a repo layout and a workflow, not an instance. Clients follow a *set* of registries and can additionally pin solver cards directly, so no repo owner is a gatekeeper or single point of failure — a solver rejected or dropped by every registry is still reachable by any client that adds its card by hand (the token-list pattern). The reference registry's per-network index is the default set: a client given no registries follows it for the wallet's network, while an explicitly empty list follows none. Keeping that default internal (rather than a table in each client) is what lets the URLs, or the fetch mechanism, move without a client change.
...
|---|---|---|
| `arkade` | the Arkade network the side settles on (`bitcoin`, `signet`, `mutinynet`, `regtest` — same vocabulary as the card's path) | `arkade:bitcoin/slip44:0` (mainnet), `arkade:mutinynet/slip44:1` (testnet) |
| `bolt11` | same Arkade-network vocabulary | `bolt11:bitcoin/slip44:0` (mainnet), `bolt11:mutinynet/slip44:1` (testnet) |
| `bitcoin` | same Arkade-network vocabulary | `bitcoin:bitcoin/slip44:0` (mainnet), `bitcoin:mutinynet/slip44:1` (testnet) |
| `eip155` | the numeric EIP-155 chain id (per CAIP-2) | `eip155:1/erc20:0x…` (an ERC-20 on Ethereum mainnet) |

The asset-namespace half of the id names WHAT settles: `slip44` is a SLIP-44-registered coin. For the three BTC-only corridors (`arkade`, `bolt11`, `bitcoin`) this is never an open coin-type space — it is SLIP-44's Bitcoin code (`0`) when the chain reference is `bitcoin` (mainnet), and SLIP-44's "Testnet (all coins)" code (`1`) on every other network (`signet`, `mutinynet`, `regtest`); the schema pattern enforces this pairing directly, so `arkade:bitcoin/slip44:1` and `arkade:mutinynet/slip44:0` are both malformed, not just semantically odd. `eip155`'s `slip44` is different: it names whichever coin type SLIP-44 assigns to *that* chain's native coin (`eip155:1/slip44:60` is ETH, since Ethereum has no address for its own coin), and stays open per chain the way `arkade`/`bolt11`/`bitcoin` deliberately don't. `asset` is an Arkade-issued asset, identified by its 68-hex-char AssetId (`arkade:bitcoin/asset:<68-hex>`) — also valid on the `bolt11`/`bitcoin` corridors, since an Arkade asset moved over Lightning or onchain (e.g. a submarine-swapped stablecoin) is a real market, not a contradiction; `erc20` is an ERC-20 contract address (`eip155:1/erc20:0x…`), valid only under `eip155`. Lowercase throughout, not EIP-55 mixed case — the id is a grouping key, and a checksum that changes the bytes would split one market into two. An erc20 reference is the TOKEN's contract address, never a swap contract's; a taker resolves the settlement contract from the corridor, not from this field.
...
A cross-asset spot market with a complete rendezvous (`discovery_pubkey` and at least one nostr relay) is quoted over the same RFQ round trip as a corridor market (`arkade:BTC` against `arkade:<asset>`, or the reverse). `quotesOverRfq` is that test. A spot card without the rendezvous stays a feed quote. Feed fields stay required either way; on a quoted market they are advisory and the quote binds.
...
```json
{
  ...
  "transports": { "nostr": { "relays": ["wss://relay.example.com", "ws://other.example.com"] } },
  "markets": [
    {
      "base_asset": { "id": "arkade:bitcoin/slip44:0", "name": "Bitcoin", "ticker": "BTC", "decimals": 8 },
...
1. For each followed registry, fetch the index for the wallet's network — `<base-url>/<network>.json` (TTL-cache ~10 min). A client given no registry list follows the reference registry's published index for that network by default — an internal default, so no client-side table is needed and the URLs may change across releases; an explicitly empty list means follow no registry, which a private deployment uses to refuse the public list, and a non-empty list replaces the default rather than merging with it. Network names follow `arkade-os/ts-sdk`; `bitcoin` is the default main Bitcoin network. Reject unknown `version` or a `network` mismatch; treat an old `generated_at` (suggested: > 7 days) as a staleness warning. Registry failures are isolated: one unreachable or invalid registry never blocks pricing from the others or from locally pinned cards.
...
Where a maker prices from the card rather than from a quote, the card is the whole pricing contract: it must carry every input the solver's own check applies, or the maker computes terms the solver then refuses.

`solver_fee` is that contract, keyed by the side the maker **deposits**. That key is the point: it fixes the units for everything beneath it, so the flat part needs no denominator and nothing is converted through the price — and because each side carries its own `bps`, the spread may differ by direction, which one market-level `fee_bps` cannot express. Exactly one key applies per swap, so the two are alternatives and never a sum.

`fee_bps` remains required and MUST equal the widest spread declared here: a reader predating `solver_fee` applies it to both directions, and understating one would have that reader compute a payout the solver refuses, after the offer is funded on chain.

`solver_fee` supersedes `fee_flat` rather than adding to it, so a card may carry both while migrating. That migration is safe for the **spread** only:

- A spread scales with the trade, so one widest `fee_bps` dominates both directions and a reader predating `solver_fee` under-asks.
- A flat has no such projection: `fee_flat` is quote-denominated while `solver_fee.base.flat` is not, and converting between them needs the price — which the card carries only as a feed.

A market declaring `solver_fee.base.flat` therefore CANNOT be priced by a reader predating that field: `fee_flat` projects the quote side alone, so such a reader understates the charge and computes exactly the payout the solver refuses once the offer is funded. Readers MUST implement `solver_fee` before pricing any market that declares one.

`charges_delivered_carrier` says whether the solver funds the dust carrier an Arkade asset rides on out of the maker's deposit. Both `solver_fee.flat` and the carrier come off the DEPOSIT before the spread, matching the solver. The carrier's amount is not on the card: it is the Arkade Service's dust, both parties to an Arkade-settled swap share that Service, and a live figure beats a cached one.
