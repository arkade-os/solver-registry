...
| `emulator_pubkey` | OPTIONAL, card-level, deployment-wide (one value for every market the solver lists) — same scope as `discovery_pubkey`. The BIP340 x-only pubkey of the "emulator," the covenant co-signer service an Arkade-covenant corridor solver uses to construct its non-interactive claim/refund script leaves. Specific to that one swap-contract mechanism — a solver running a different corridor mechanism, or a plain arkade-arkade spot market, has no use for it — so it is never required, even on a corridor market, and is independent of the `discovery_pubkey`/`sig`/`transports` rendezvous requirement below. |
...
2. Flattens the network's cards into one market list, each entry carrying its solver's `name` (and `discovery_pubkey`/`emulator_pubkey`/`transports` when present; `sig` stays in the card, it is not propagated).
