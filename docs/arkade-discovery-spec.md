...
A market MAY declare `methods` to say which of the two it actually serves, and a client MUST honour it rather than infer the method from the market's shape. Inference is what step 5 describes, and it is only a proxy: an arkade-to-arkade market looks like a spot market and may nonetheless be served over RFQ, or over offer packets, or both. Getting it wrong is not symmetric — an RFQ for an unserved market costs one refused round trip, while an offer packet for one is funded ON CHAIN first and then ignored, leaving the maker to cancel a deposit no solver will take. Absent `methods` means UNDECLARED rather than none, so cards written before the field keep the inferred behaviour below.
